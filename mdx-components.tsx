...
export function useMDXComponents(components: MDXComponents): MDXComponents {
	return {
		...
		Step: ({ className, children, ...props }: React.ComponentProps<"h3">) => (
			<h3 id={generateId(children?.toString() || "")} className={cn("step", className)} data-heading="3" {...props}>
				{children}
			</h3>
		),
		...
		thead: ({ className, ...props }: React.ComponentProps<"thead">) => (
			<thead className={cn("bg-zinc-100 text-zinc-900 dark:bg-zinc-900 dark:text-zinc-100", className)} {...props} />
		),
		tbody: ({ className, ...props }: React.ComponentProps<"tbody">) => (
			<tbody className={cn("divide-y divide-zinc-200 dark:divide-y dark:divide-zinc-600", className)} {...props} />
		),
		tr: ({ className, ...props }: React.ComponentProps<"tr">) => <tr className={cn("h-10", className)} {...props} />,
		th: ({ className, ...props }: React.ComponentProps<"th">) => (
			<th className={cn("px-4 pb-0 text-left align-middle font-[450]", className)} {...props} />
		),
		td: ({ className, ...props }: React.ComponentProps<"td">) => (
			<td className={cn("px-4 py-2 text-left align-middle", className)} {...props} />
		),
